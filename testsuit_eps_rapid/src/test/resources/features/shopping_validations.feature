...
#      | property_id       |             20321 |
  ...
  @rapid_test
  Scenario Outline: <test_case> Availability API  Rapid test Header "<header>" with "<value>"
    ...
    Examples: 
      ...
      |[AVAIL2]| Test   | no_availability        |  404 | no_availability        | No availability was found for the properties requested.            |
      |[AVAIL3]| Test   | unknown_internal_error |  500 | unknown_internal_error | An internal server error has occurred.                             |
      |[AVAIL4]| Test   | service_unavailable    |  503 | service_unavailable    | This service is currently unavailable.                             |
      |[AVAIL1]| Test   | forbidden              |  403 | request_forbidden      | Your request could not be authorized. Ensure that you have access. |

  @rapid_test
  Scenario: [AVAIL55] Availability API Rapid test with invalid value like "Test=INVALID"
    Given Basic web application is running
  ...
  @data_test
  Scenario Outline: <test_case> Availability API missing Query Param "<query_param>"
    Given Basic web application is running
    ...
    Examples: 
      ...
      |[AVAIL5]| property_id       | property_id.required       | Property Id is required.                                                                                                   |
      |[AVAIL22]| checkin           | checkin.required           | Checkin is required.                                                                                                       |
      |[AVAIL22]| checkout          | checkout.required          | Checkout is required.                                                                                                      |
      |[AVAIL31]| currency          | currency.required          | Currency code is required.                                                                                                 |
      |[AVAIL34]| language          | language.required          | Language code is required.                                                                                                 |
      |[AVAIL36]| country_code      | country_code.required      | Country code is required.                                                                                                  |
      |[AVAIL37]| occupancy         | occupancy.required         | Occupancy is required.                                                                                                     |
      |[AVAIL44]| sales_channel     | sales_channel.required     | Sales Channel is required.  Accepted sales_channel values are: [website, agent_tool, mobile_app, mobile_web, cache, meta]. |
  ...
  Scenario Outline:<test_case> Availability API with "<scenario>" for query Param "checkin"
    ...
    Examples: 
      |test_case| scenario            | days | error_type                       | error_message                  |
      |[AVAIL23]| past-dates          |   -2 | checkin.invalid_date_in_the_past | Checkin cannot be in the past. |
      |[AVAIL25]| Too-Advance-Checkin |  510 | checkin.invalid_date_too_far_out | Checkin too far in the future. |

  @data_test
  Scenario:[AVAIL24] Availability API with Query Param "checkin" date greater than Query Param "checkout" date
    Given Basic web application is running
  ...
  @data_test
  Scenario Outline: <test_case> Availability API with "<scenario>" for Query Param "occupancy"
    Given Basic web application is running
    ...
    Examples: 
      ...
      #|[AVAIL27] | more than 8 people in a room  |          9 |             0 | number_of_adults.invalid_above_maximum   | Number of adults must be less than 9.    |     9 |
     |[AVAIL30] | no adult in a room            |          0 | 2,3,4,5,6,7,8 | number_of_adults.invalid_below_minimum   | Number of adults must be greater than 0. |     0 |
      |[AVAIL28]| chid age with gerater than 18 |          2 |            20 | child_age.invalid_outside_accepted_range | Child age must be between 0 and 17.      |    20 |

  @data_test
  Scenario Outline:<test_case> Availability API with invalid  data format for Query Param "<query_param>"
    Given Basic web application is running
    ...
    Examples: 
      |test_case| query_param       | value | error_type                | error_message                                                                                                                                                                                                                                                                                      |
      |[AVAIL32]| currency          | RRR   | currency.not_supported    | Currency is not supported. Supported currencies are: [AED, ARS, AUD, BRL, CAD, CHF, CNY, DKK, EGP, EUR, GBP, HKD, IDR, ILS, INR, JPY, KRW, MXN, MYR, NOK, NZD, PHP, PLN, RUB, SAR, SEK, SGD, THB, TRY, TWD, USD, VND, ZAR]                                                                              |
      |[AVAIL34]| language          | as-US | language.not_supported    | Language is not supported. Supported languages are: [ar-SA, cs-CZ, da-DK, de-DE, el-GR, en-US, es-ES, es-MX, fi-FI, fr-CA, fr-FR, hr-HR, hu-HU, id-ID, is-IS, it-IT, ja-JP, ko-KR, lt-LT, ms-MY, nb-NO, nl-NL, pl-PL, pt-BR, pt-PT, ru-RU, sk-SK, sv-SE, th-TH, tr-TR, uk-UA, vi-VN, zh-CN, zh-TW] |
      |[AVAIL35]| country_code      | RRR   | country_code.invalid      | Country code is invalid.                                                                                                                                                                                                                                                                           |
      |[AVAIL51]| rate_option       | test  | rate_option.invalid       | Rate Option is invalid.  Accepted rate_option values are: [net_rates, closed_user_group].                                                                                                                                                                                                          |
  ...
  @business_test
  Scenario: [AVAIL16] Availability API response validation matching occupancies with single Query Param "occupancy" and "property_id"
    Given Basic web application is running
    And run shopping
    Then the response code for "SHOPPING" should be 200
  ...
  @business_test
  Scenario: [AVAIL16] Availability API response validation matching occupancies with multiple Query Param "occupancy" and single Query Param "property_id"
    Given Basic web application is running
    When set "SHOPPING" queryParam "occupancy" value "null"
  ...
  @business_test
  Scenario:[AVAIL52] Availability API response validation matching occupancies with single Query Param "occupancy" and multiple Query Param "property_id"
    Given Basic web application is running
    When set "SHOPPING" queryParam "property_id" value "null"
  ...
  @business_test
  Scenario:[AVAIL52] Availability API response validation matching occupancies with multiple Query Param "occupancy" and multiple Query Param "property_id"
    Given Basic web application is running
    When set "SHOPPING" queryParam "property_id" value "null"
  ...
  @business_test
  Scenario: [AVAIL29] Availability API response validation with child age 0
    Given Basic web application is running
  ...
  @business_test
  Scenario:[AVAIL7] Availability API response validation for "available_rooms"
    Given Basic web application is running
    And run shopping
    Then the response code for "SHOPPING" should be "200"
  ...
  @business_test
  Scenario:[AVAIL13] Availability API response validation for "amenities"
    Given Basic web application is running
    And run shopping
    Then the response code for "SHOPPING" should be "200"
  ...
  @business_test
  Scenario:[AVAIL14] Availability API response validation for "fenced_deal"
    Given Basic web application is running
    And run shopping
    Then the response code for "SHOPPING" should be "200"
  ...
  @business_test
  Scenario:[AVAIL20] Availability API response validation for "deposit_policies" if true
    Given Basic web application is running
    And run shopping
    Then the response code for "SHOPPING" should be "200"
  ...
  @business_test
  Scenario:[AVAIL17] Availability API response validation for "STAY_PRICE_TYPE"
    Given Basic web application is running
    And run shopping
    Then the response code for "SHOPPING" should be "200"
  ...
  @business_test
  Scenario:[AVAIL8] Availability API response validation for "NIGHTLY_PRICE"
    Given Basic web application is running
  ...
  @business_test
  Scenario:[AVAIL12] Availability API response validation for "total_price"
    Given Basic web application is running
    And run shopping
    Then the response code for "SHOPPING" should be "200"
  ...
  @business_test
  Scenario:[AVAIL9] Availability API response validation for "CURRENCY_CODE"
    Given Basic web application is running
  ...
  @business_test
  Scenario:[AVAIL10] Availability API response validation for "BILLABLE_CURRENCY"
    Given Basic web application is running
    And set "SHOPPING" queryParam "currency" value "INR"
    And run shopping
    Then the response code for "SHOPPING" should be "200"
